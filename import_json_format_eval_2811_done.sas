...
libname jsonlib json "C:\Users\raffelt\jsons_with_sas\patient_data_export1411.json";

/*this lib contains exclusively my output tables, where each table corresponds to one test*/
libname out "C:\Users\raffelt\jsons_with_sas\clean_tables";
run;
...
/*create a table for missing values to be stored in, maybe easier to get later?*/
proc sql;
...
/*TRY MERGING ON CRFS_ITEMS (NUMERIC) TO GET THE CORRECT MISSINGS -> THEN*/
...
/*
data crfs_items;
	merge crfs_items(in=a) t_missings(in=b);
	by value;
	if b then value = special_missing;
	drop special_missing format_comment;
run;
*/

/*change crfnames/ variablenames to replace "-",":" with "" -> easy sorting later*/
data old_crfs;
set crfs;
run;
data crfs;
set crfs;
crf_name = compress(translate(translate(crf_name, "","-"),"",":"));
run;
...
proc sql noprint;
	...
	from unique_crfs
	where upcase(crf_name) not like upcase('DSGVO_Consent');
quit;
...
%macro add_evaluation(tab_name);
	proc sql;
			...
			select * from &tab_name join crfs_evaluation on &tab_name..ordinal_crfs = crfs_evaluation.ordinal_crfs; /*double periods because sas tries to concat on period for some reason. piece of garbage*/
	quit;
...
%macro add_labels_to_vars(tab_name);
	...
	proc sql noprint;
	    select catx('=', name, quote(trim(field_label)))
...
%macro pretty_table(tab_name);
	...
	/*this is supposed to delete the table from the work directory, but it doesn't work*/
	proc datasets library library=work nolist;
		delete &tab_name;
	quit;

%mend;

/*IMPORTANT - REMOVES HYPHENS FROM TEST NAMES WHEN CREATING A NEW TABLE:
EXAMPLE: bfi-10 becomes bfi10*/
%macro create_tables;
	...
	*options nonotes;
    ...
    %do i = 1 %to &n_tables;
        ...
        /* Truncate table name if longer than 32 characters */
        %if %sysfunc(lengthn(&table_name)) > 32 %then %do;
            %let table_name = %substr(&table_name, 1, 32);
        %end;
